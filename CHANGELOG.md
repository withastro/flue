...
- **Node: Process agent submissions concurrently.** The Node coordinator no longer serializes all submission processing through a single loop. Submissions targeting different sessions now process in parallel via a concurrent claim loop. Two users talking to different agents no longer block each other.
- **Node: Graceful shutdown on SIGINT/SIGTERM.** The server now drains active agent work before exiting. Active LLM turns and tool calls finish at the turn boundary (no new turns start), the coordinator waits up to 30 seconds for settlement, WebSocket clients receive a `1001 Going Away` close frame, and the process exits with standard Unix signal codes (130 for SIGINT, 143 for SIGTERM). Submissions that don't settle within the timeout are reclaimed on next startup via lease expiry.
- **Node: Lease-based submission ownership.** Claimed submissions now carry an `owner_id` and `lease_expires_at` timestamp, renewed by a 10-second heartbeat. After a crash, only submissions with expired leases are reconciled — replacing the previous assumption that all running submissions are orphaned. This is the foundation for future multi-node support where multiple coordinators compete for claims against a shared database.
- **Node: Durable admission for all direct prompts.** Direct agent prompts (HTTP, SSE, WebSocket, and local CLI) now always enter the durable submission lifecycle. The previous inline execution path and its process-local session lock (`activeAttachedAgentSessions`) have been removed. All prompts share the same admission, claim, journal, and settlement flow as dispatches.
- **Postgres: Persist workflow run history across restarts.** `PersistenceAdapter` now requires `connectRunStore()` and `connectRunRegistry()`. The Postgres adapter returns SQL-backed implementations with `flue_runs`, `flue_run_events`, and `flue_run_registry` tables. Workflow run history, events, and the run index are now durable for Postgres users. The built-in SQLite adapter continues to use in-memory storage.
...
- **Build: Escape generated import paths.** Generated Node and Cloudflare entry imports now serialize source file paths safely, so project paths containing quotes, backslashes, or other special characters (such as `/Users/o'brien/`) no longer produce a broken build.
...
- **Cloudflare: Extend generated workflow Durable Objects.** Workflow modules may now export `cloudflare = extend({ base, wrap })` from `@flue/runtime/cloudflare` to add native Agents SDK lifecycle hooks beneath Flue-owned routing or wrap the final generated workflow Durable Object class with integrations such as Sentry.
