...
- **Attachments are separate immutable payloads.** Canonical records carry opaque `{ id, mimeType, size, digest }` references resolved through the required `AttachmentStore`; the public conversation projects them as `file` parts with media type only (no bytes or download URL).
