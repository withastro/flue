...
- **`session.spawn(...)` opens a multi-turn conversation with a subagent.** Until now the only way to reach a subagent was `session.task()`, which creates a child session, delegates a single prompt, and disposes it — there was no way to keep talking to the child. `spawn(options?)` returns a `SubagentHandle` whose lifecycle the caller owns: it exposes the child session's `prompt` / `skill` / `task` / `shell` / `fs` surface and stays alive, retaining context across calls, until `close()` (or `await using` disposal). Pass `options.agent` to select a declared subagent profile, or omit it to reuse the parent's configuration in a fresh conversation. The child is parent-owned in the conversation topology exactly like a `task()` child — not a separately addressable endpoint — and, like a programmatic `task()`, carries no parent `task` tool call, so it lives only for the current process and is not restored by durable subagent recovery. `SpawnOptions` and `SubagentHandle` are exported from `@flue/runtime`.
