...
The timeout is enforced preemptively, not just between attempts. At the deadline the running attempt's abort signal fires, so work suspended on a signal-aware await (a provider call, a sandbox command, any tool — a `run` that ignores its signal is abandoned rather than awaited) unwinds and settles through the normal paths — and an attempt hung below the abandonable layer is settled `failed` over it, its late writes fenced off. A hang delays settlement by at most the deadline plus one wake; it can never strand the submission. Most stalls never reach the deadline at all: a model stream that goes silent past its [idle timeout](/docs/reference/provider-api/#cloudflarebindingprovider) fails as a transient provider error and the turn retries under the [error budget](#recovery-after-an-interruption).
...
On Cloudflare, every agent conversation is a Durable Object with its own SQLite storage, so ownership is structural — the platform guarantees one live instance per conversation, and there is no lease protocol to operate. Execution is one durable state machine per conversation on the Agents SDK's Tasks engine, and recovery is wake-driven:

- **Wake on start.** Whenever the Durable Object starts — after an eviction, a code deploy, or a platform reset — Flue wakes the conversation machine if any submission is unsettled. Its `idle` pass reconciles first: an attempt that was running when the previous instance died is classified from the canonical stream and settled or given a replacement attempt before new work is claimed. A `turn` interrupted mid-response replays on the same start and reconciles from the same evidence.
- **A durable claim.** While a turn is live, the SDK keeps a claim deadline in storage and wakes the object at it, so an interrupted submission recovers promptly even if no external request ever arrives. The submission's wall-clock timeout aborts the turn's work from inside it; a turn that then stops making progress trips the SDK's transition watchdog, which abandons that attempt — its later writes to the conversation are refused — and returns the machine to `idle`, where the submission settles through the timeout classification above.
