...
Accepted work runs inside the Durable Object itself, as one durable state machine on the Agents SDK's [Tasks](https://github.com/cloudflare/agents/blob/main/docs/agents/tasks.md) engine. Every conversation has one `flue:conversation@v1` run at a fixed address: its `idle` phase parks on the run's mailbox — holding no alarm at all — until admission sends it a wake, then claims the next runnable submission from the durable queue; its `turn` phase awaits that submission's full response — every model turn and tool call — and settles it, and the machine returns to `idle`, where the next queued head is claimed in the same invocation. Admission always returns first; the response begins in the same invocation when the object is warm, or on the next wake otherwise. The SDK owns the durable claim while a turn is live (a backstop that survives eviction), replays the turn on a fresh isolate after an interruption, and bounds a turn that stops making progress with its transition watchdog; the submission's own [durability timeout](/docs/guide/durability/#retry-budget-and-timeout) aborts the turn's work from inside it. Three consequences:
