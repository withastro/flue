...
export class CloudflarePlugin implements BuildPlugin {
	name = 'cloudflare';
	protected usesPackagedSkills = false;
	...
	bundle: BuildPlugin['bundle'] = 'none';
    ...
    if (typeof super.onFiberRecovered === 'function') {
      return super.onFiberRecovered(ctx);
		...
		const userAppImport = appEntry
			...
			: '';
		const packagedSkillsImport = this.usesPackagedSkills ? `import { getPackagedSkills } from 'virtual:flue/packaged-skills';` : '';
		const packagedSkillsValue = this.usesPackagedSkills ? 'getPackagedSkills()' : '{}';
...
`;
	...
	async additionalOutputs(ctx: BuildContext): Promise<Record<string, string>> {
		...
		const additions: FlueAdditions = {
			...
			main: this.usesPackagedSkills ? '_entry.ts' : '_entry.bundled.js',
...
export class ViteCloudflarePlugin extends CloudflarePlugin {
	name = 'cloudflare-vite';
	bundle: BuildPlugin['bundle'] = 'vite-cloudflare';
	protected usesPackagedSkills = true;
}
