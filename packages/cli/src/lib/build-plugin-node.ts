...
export class NodePlugin implements BuildPlugin {
	...
	generateEntryPoint(ctx: BuildContext): string {
		...
		const packagedSkillsImport = this.bundle === 'vite' ? `import { getPackagedSkills } from 'virtual:flue/packaged-skills';` : '';
		const packagedSkillsValue = this.bundle === 'vite' ? 'getPackagedSkills()' : '{}';
		return `
