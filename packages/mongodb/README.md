...
Call and await `migrate()` before `connect()`. Migration verifies exact validators and required indexes before writing the Flue schema version, rejects newer versions, and retains permanent session guard documents to serialize admission, deletion, and same-session claims even when no submission exists. Flue stores session generations, durable submissions and journals, leases, deletion receipts, runs, event streams, and image chunks. Arbitrary values are staged as bounded immutable parts with durable generation state before a short transaction publishes their manifest; images remain in the runtime `PersistedChunkStore` representation. Session deletion uses a leased monotonic fence. The external session-tree callback must be idempotent: a crashed owner can be taken over only after its lease expires, the callback may run again, and admission remains blocked until the active fenced cleanup completes.
