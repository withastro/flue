  ...
  async onFiberRecovered(ctx) {
    ...
    if (typeof super.onFiberRecovered === 'function') {
      return super.onFiberRecovered(ctx);
		...
		const { config: userConfig } = await this.getUserConfig(ctx.workspaceDir);
...
`;
	...
	async additionalOutputs(ctx: BuildContext): Promise<Record<string, string>> {
		...
		// lives at workspaceDir (the project root) and is never modified; the
		...
		const additions: FlueAdditions = {
			defaultName: path.basename(ctx.workspaceDir) || 'flue-agents',
		...
		if (sandboxClassNames.length > 0) {
			assertSandboxPackageInstalled(sandboxClassNames, ctx.workspaceDir);
