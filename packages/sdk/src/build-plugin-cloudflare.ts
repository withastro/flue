    ...
    if (typeof super.onFiberRecovered === 'function') {
      return super.onFiberRecovered(ctx);
		...
		const sandboxReExports = sandboxClassNames
			...
			.join('\n');
		// When the user has @cloudflare/sandbox bindings, also import the class
		// locally so resolveSandbox() can use a positive `instanceof` check
		// against the RPC stub returned by getSandbox(). This resolves against
		// the user's installed copy of @cloudflare/sandbox (the same module
		// instance their agent code imports from), so prototype identity holds.
		// Workerd RPC stubs are Proxies that lie about `in` and `typeof`, so
		// structural duck-typing against them is unreliable; `instanceof` walks
		// the prototype chain and is unaffected by the proxy's traps.
		const sandboxClassImport =
			sandboxClassNames.length > 0
				? `import { Sandbox as __FlueCfSandbox } from '@cloudflare/sandbox';`
				: '';
...
function resolveSandbox(sandbox) {
  ${
		sandboxClassNames.length > 0
			? `if (sandbox instanceof __FlueCfSandbox) {
