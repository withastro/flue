...
import { createRegistryOps, handleRegistryRequest, type SqlStorage } from '../src/cloudflare/registry-ops.ts';
...
function makeFakeSql(): SqlStorage {
	...
	return {
		exec(query: string, ...bindings: unknown[]) {
			...
			return { toArray() { return rows as Record<string, unknown>[]; } };
		},
	};
}
...
describe('createRegistryOps (SQL paths)', () => {
	it('round-trips a workflow pointer through recordRunStart and lookupRun', () => {
		...
		expect(ops.lookupRun(runId)).toEqual({ runId, owner: owner('hello', runId), status: 'active', startedAt: STARTED_AT_1, endedAt: undefined, durationMs: undefined, isError: undefined });
	...
	it('records terminal state for workflow pointers', () => {
		...
		assert.ok(pointer);
		expect(pointer).toMatchObject({ status: 'completed', endedAt: ENDED_AT, durationMs: 12345, isError: false });
	});
	...
	it('silently drops terminal updates without a prior workflow start', () => {
		const ops = createRegistryOps(makeFakeSql());
		ops.recordRunEnd({ runId: 'workflow:hello:orphan', endedAt: ENDED_AT, durationMs: 0, isError: false });
		expect(ops.lookupRun('workflow:hello:orphan')).toBeNull();
	});

	it('sorts workflow runs descending and composes workflow filters', () => {
		const ops = createRegistryOps(makeFakeSql());
		for (const [runId, workflowName, startedAt] of [['workflow:hello:a', 'hello', STARTED_AT_1], ['workflow:hello:b', 'hello', STARTED_AT_2], ['workflow:world:c', 'world', STARTED_AT_3]] as const) {
			ops.recordRunStart({ runId, owner: owner(workflowName, runId), startedAt });
		}
		ops.recordRunEnd({ runId: 'workflow:hello:a', endedAt: ENDED_AT, durationMs: 1, isError: false });
		expect(ops.listRuns({}).runs.map((run) => run.runId)).toEqual(['workflow:world:c', 'workflow:hello:b', 'workflow:hello:a']);
		expect(ops.listRuns({ workflowName: 'hello' }).runs.map((run) => run.runId)).toEqual(['workflow:hello:b', 'workflow:hello:a']);
		expect(ops.listRuns({ status: 'active' }).runs.map((run) => run.runId)).toEqual(['workflow:world:c', 'workflow:hello:b']);
		expect(ops.listRuns({ workflowName: 'hello', status: 'completed' }).runs.map((run) => run.runId)).toEqual(['workflow:hello:a']);
	});
	...
	it('paginates workflow run pointers', () => {
		...
		for (let i = 0; i < 5; i++) {
			...
			ops.recordRunStart({ runId, owner: owner('hello', runId), startedAt: `2026-05-13T10:${String(i).padStart(2, '0')}:00.000Z` });
		}
	...
	it('rejects workflow owners whose instance id does not match run id', () => {
		const ops = createRegistryOps(makeFakeSql());
		expect(() => ops.recordRunStart({ runId: 'workflow:daily-report:01A', owner: owner('daily-report', 'workflow:daily-report:01B'), startedAt: STARTED_AT_1 })).toThrow(/same instanceId/);
	});

	it('does not surface historical agent-owned pointer rows', () => {
		const sql = makeFakeSql();
		const ops = createRegistryOps(sql);
		sql.exec(`INSERT INTO flue_registry_runs (run_id, owner_kind, agent_name, instance_id, status, started_at) VALUES (?, 'agent', ?, ?, 'active', ?)`, 'legacy-agent', 'hello', 'inst_a', STARTED_AT_1);
	...
	it('falls back to page 1 on a malformed cursor', () => {
		...
		for (let i = 0; i < 3; i++) {
			...
			ops.recordRunStart({ runId, owner: owner('hello', runId), startedAt: `2026-01-01T00:00:0${i}.000Z` });
		}
...
describe('handleRegistryRequest (REST router)', () => {
	it('gets workflow pointers and returns 404 for missing values', async () => {
		...
		ops.recordRunStart({ runId, owner: owner('hello', runId), startedAt: STARTED_AT_1 });
		const hit = await handleRegistryRequest(ops, new Request(`https://registry/pointers/${encodeURIComponent(runId)}`));
	...
	it('starts and ends encoded workflow pointers through private routes', async () => {
		...
		const start = await handleRegistryRequest(ops, new Request(`https://registry/pointers/${encoded}/start`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ owner: owner('daily-report', runId), startedAt: STARTED_AT_1 }) }));
		expect(start.status).toBe(204);
		const end = await handleRegistryRequest(ops, new Request(`https://registry/pointers/${encoded}/end`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ endedAt: ENDED_AT, durationMs: 5000, isError: true }) }));
	...
	it('lists workflow pointers with workflow query filtering', async () => {
		...
		for (const name of ['hello', 'world']) {
			...
			ops.recordRunStart({ runId, owner: owner(name, runId), startedAt: name === 'hello' ? STARTED_AT_1 : STARTED_AT_2 });
		}
		const listFiltered = await handleRegistryRequest(ops, new Request('https://registry/pointers?workflow=hello'));
		expect(((await listFiltered.json()) as { runs: { runId: string }[] }).runs.map((run) => run.runId)).toEqual(['workflow:hello:list']);
		const removedInstancesRoute = await handleRegistryRequest(ops, new Request('https://registry/instances'));
