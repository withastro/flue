...
If `./.flue/` exists, Flue reads sources from there; otherwise it reads from the workspace root. The two layouts never mix. `flue build` always writes to `./dist/` at the workspace root, regardless. `wrangler.jsonc` and any `Dockerfile` you ship live at the workspace root too. Examples in this guide use the `./.flue/` layout — drop the prefix if you prefer the bare layout.
